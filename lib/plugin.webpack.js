...
class AutoRouterWebPackPlugin {
    constructor({ pagePath, output, filename, defaultLazyImport, }) {
        if (!pagePath) {
        ...
        this.output = output || ".";
        this.defaultLazyImport = !!defaultLazyImport;
    }
    apply(compiler) {
    ...
    buildRouter() {
        ...
        process.exec(`${parse_exe} parse -o ${this.output} -r ${this.pagePath} -n ${this.filename} ${this.defaultLazyImport ? "-i" : ""}`, (err, stdout, stderr) => {
            // console.log(stdout);
        });
